...
이 프로젝트는 Google Colab 환경에서 Python을 사용하여 진행되었으며, Pytorch 프레임워크의 라이브러리 활용을 위해 GPU or TPU 환경에서의 CUDA 사용이 필수적입니다.<br/>
더 자세한 분석 방법 및 결과는 다음과 같은 추가 코드 파일 및 리포트에서 확인하실 수 있습니다.<br/> 
- [해당 연구에 대한 code 보러가기](project.ipynb) (현재 개인적인 이유로 비공개 상태이며, 원하는 경우 [portfolio](https://naayeong-kim.github.io/) 페이지 하단의 이메일로 요청 바랍니다)<br/>
...
결과적으로 80% 이상의 정확도로 흑색종을 식별할 수 있었지만, 제한적인 데이터 및 GPU 메모리 등의 이유로 충분한 학습이 불가능했기에 백본 모델에 비해 어텐션 메커니즘을 적용한 모델에서 눈에 띄는 성능 향상을 얻지는 못했습니다. 그러나 이 연구는 주로 자연어처리(NLP)에서만 사용되던 기술을 컴퓨터 비전 분야에 녹여내는 방법에 대한 통찰을 제시합니다. 더 자세한 결과 및 고찰은 [full report](report.pdf)를 참조하시기 바랍니다.
